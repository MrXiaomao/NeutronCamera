...
void CpsStatisticsWindow::initUi()
{
    ...
    ui->toolButton_start->setDefaultAction(ui->action_cps);
...
// 计数率统计
void CpsStatisticsWindow::cpsStatistics()
{
    //提取有效波形参数
...
void CpsStatisticsWindow::on_action_cps_triggered(bool checked)
{
    // 计数率统计
    cpsStatistics();
}
...
void CpsStatisticsWindow::initHeatmap()
{
    ...
    allAxes << timeCountsAxisRect->axes() << timeChannelCountsAxisRect->axes() << channelCountsAxisRect->axes();
    ...
    QVBoxLayout* vLayout = new QVBoxLayout(ui->spectroMeterPageInfoWidget_cps);
    vLayout->addWidget(customPlot);
    ui->spectroMeterPageInfoWidget_cps->setLayout(vLayout);
...
void CpsStatisticsWindow::replyCpsPlot(QMap<quint8/*通道号*/, QMap<quint16/*时刻*/,quint32/*计数率*/>> mapPairs)
{
    ...
    for (auto iter = mapPairs.begin(); iter != mapPairs.end(); ++iter){
        ...
        QCPGraph *graph = nullptr;
        if (channel < 11)
            graph = mCpsPlot->graph(QStringLiteral("HC %1").arg(channel));
        else
            graph = mCpsPlot->graph(QStringLiteral("VC %1").arg(channel));
        graph->setData(xData, yData);
    }
...
void CpsStatisticsWindow::on_comboBox_h5Files_currentTextChanged(const QString &arg1)
{
    ...
    {
        ui->line_measure_startT->setText(QString::number(packerStartTime));
