...
//////////////////////////////////////////////////////////////////////
    ...
    splitterH1->addWidget(ui->centralVboxTabWidget);
...
void OfflineWindow::initCustomPlot(int index, QCustomPlot* customPlot, QString axisXLabel, QString axisYLabel, int graphCount/* = 1*/)
{
...
void OfflineWindow::replyWaveform(quint8 timestampIndex, quint8 cameraOrientation, QVector<quint16>& waveformBytes)
{
    ...
    if (timestampIndex == 1)
        customPlot = ui->spectroMeter_top;
    else if (timestampIndex == 2)
        customPlot = ui->spectroMeter_central;
    else if (timestampIndex == 3)
        customPlot = ui->spectroMeter_bottom;
    else
        return;
...
void OfflineWindow::replyNeutronSpectrum(quint8, QVector<quint16>& spectrumBytes)
{

}

void OfflineWindow::replyGammaSpectrum(quint8, QVector<quint16>& spectrumBytes)
{
...
void OfflineWindow::loadRelatedFiles(const QString& filePath)
{
    ...
    // quint32 index =1;
    // quint32 packref = 1;
    // auto analyzeFile = [=](const QString& _filePath, quint32& _index, quint32& _packref){
    //     QRegularExpression re("\\d+"); // \d+ 匹配一个或多个数字
    //     auto matches = re.globalMatch(QFileInfo(_filePath).baseName());
    //     QStringList numbers;
    ...
    //     }
    //     _index = numbers.at(0).toUInt();
    //     _packref = numbers.at(1).toUInt();
    // };
    // analyzeFile(filePath, index, packref);
    ...
    //         quint32 _index =1;
    //         quint32 _packref = 1;
    //         analyzeFile(filePath, _index, _packref);
    //         if (index == _index){
    //             // 这里应该对文件进一步解析，得到时间范围

    //             int row = ui->tableWidget_file->rowCount();
    //             ui->tableWidget_file->insertRow(row);
    //             ui->tableWidget_file->setItem(row, 0, new QTableWidgetItem(filePath));
    //         }
    //     }
    // }
}
...
void OfflineWindow::on_action_analyze_triggered()
{
    ...
    //这类假设每个文件能谱时长为50ms
    quint32 t1 = (float)(ui->spinBox_timeNode1->value() + 49)/ 50;
    quint32 t2 = (float)(ui->spinBox_timeNode2->value() + 49)/ 50;
    quint32 t3 = (float)(ui->spinBox_timeNode3->value() + 49)/ 50;

    //t1-水平相机
    {
        QString filePath = QString("%1/%2data%3.bin").arg(ui->textBrowser_filepath->toPlainText()).arg(ui->comboBox_horCamera->currentIndex()+1).arg(t1);
        mPCIeCommSdk.setCaptureParamter(1, 12, 10, 20, 30);
    ...
    //t1-垂直相机
    {
        QString filePath = QString("%1/%2data%3.bin").arg(ui->textBrowser_filepath->toPlainText()).arg(ui->comboBox_verCamera->currentIndex()+12).arg(t1);
        mPCIeCommSdk.setCaptureParamter(1, 12, 10, 20, 30);
        if (!mPCIeCommSdk.openHistoryData(filePath))
        {
            QMessageBox::information(this, tr("提示"), tr("文件格式错误，加载失败！"));
