    ...
    , mainWindow(static_cast<QGoodWindowHelper*>(parent))
{
    ...
    connect(&mPCIeCommSdk, &PCIeCommSdk::doWaveform, this, &OfflineWindow::onWaveform);
    ...
    connect(this, &OfflineWindow::doWaveform, this, &OfflineWindow::onWaveform);
    connect(this, SIGNAL(doWriteLog(const QString&,QtMsgType)), this, SLOT(onWriteLog(const QString&,QtMsgType)));
    connect(this, SIGNAL(doCalculateDensityPSD(quint8,QVector<QPair<double,double>>&)), this, SLOT(onCalculateDensityPSD(quint8,QVector<QPair<double,double>>&)));
...
void OfflineWindow::initUi()
{
...

    {
        ...
        actionGrp->addAction(ui->action_cps);
    ...
    //布局
    {
        ...
        {
            {
                QSplitter *splitterV1 = new QSplitter(Qt::Vertical,ui->spectroMeterPageInfoWidget_page_1);
                splitterV1->setHandleWidth(2);
                splitterV1->addWidget(ui->spectroMeter_waveform_LSD);
                splitterV1->addWidget(ui->spectroMeter_neutron_spectrum_LSD);
                splitterV1->addWidget(ui->spectroMeter_gamma_spectrum_LSD);
                splitterV1->setSizes(QList<int>() << 100000 << 100000 << 100000);
                splitterV1->setCollapsible(0,false);
                splitterV1->setCollapsible(1,false);
                ui->spectroMeterPageInfoWidget_page_1->layout()->addWidget(splitterV1);
            }

            {
                QSplitter *splitterH1 = new QSplitter(Qt::Horizontal,ui->spectroMeterPageInfoWidget_page_2);
                ...
                QSplitter *splitterH2 = new QSplitter(Qt::Horizontal,ui->spectroMeterPageInfoWidget_page_2);
                ...
                QSplitter *splitterV1 = new QSplitter(Qt::Vertical,ui->spectroMeterPageInfoWidget_page_2);
                splitterV1->setHandleWidth(2);
                splitterV1->addWidget(splitterH1);
                splitterV1->addWidget(splitterH2);
                ui->spectroMeterPageInfoWidget_page_2->layout()->addWidget(splitterV1);
            }
        }
        {
            ...
            splitterV1->addWidget(ui->spectroMeter_neutron_spectrum_PSD);
            splitterV1->setSizes(QList<int>() << 100000 << 100000 << 100000);
            splitterV1->setCollapsible(0,false);
            splitterV1->setCollapsible(1,false);
        ...
        }
        {
            ...
            splitterV1->addWidget(ui->spectroMeter_gamma_spectrum_LBD);
            splitterV1->setSizes(QList<int>() << 100000 << 100000 << 100000);
            splitterV1->setCollapsible(0,false);
            splitterV1->setCollapsible(1,false);
            splitterV1->setCollapsible(2,false);
...

    {
        ...
        initCustomPlot(ui->spectroMeter_gamma_spectrum_LBD, tr("道址"), tr("伽马能谱"));
...
void OfflineWindow::initCustomPlot(QCustomPlot* customPlot, QString axisXLabel, QString axisYLabel)
{
    ...
    if (customPlot == ui->spectroMeter_waveform_PSD ||
        customPlot == ui->spectroMeter_neutron_spectrum_PSD ||
        customPlot == ui->spectroMeter_waveform_LBD ||
        customPlot == ui->spectroMeter_gamma_spectrum_LBD ||
        customPlot == ui->spectroMeter_waveform_LSD ||
        customPlot == ui->spectroMeter_neutron_spectrum_LSD ||
        customPlot == ui->spectroMeter_gamma_spectrum_LSD){
...
void OfflineWindow::onWriteLog(const QString &msg, QtMsgType msgType/* = QtDebugMsg*/)
{
#if 0
    // 创建一个 QTextCursor
...
void OfflineWindow::loadRelatedFiles(const QString& dirPath)
{
    ...
    if (data_start != -1) {
        ...
        ui->spinBox_time1->setMinimum((start_tm-1)*time_per);
        ui->spinBox_time1->setMaximum((start_tm-1)*time_per+measureTime);

        ui->spinBox_startT->setValue((start_tm-1)*time_per);
        ui->spinBox_endT->setValue(start_tm*time_per);
    }
    else {
...
{
    if(ui->action_waveform->isChecked()){
        // 波形显示
        wafeformShow();
...
void OfflineWindow::ngammaFilter()
{
    ...
    plotTimer.start();
    emit doPSDPlot(PCIeCommSdk::CameraOrientation::Horizontal, energyVec, psdVec, denDouble);
...
// 波形显示
void OfflineWindow::wafeformShow()
{
    QList<QCustomPlot*> listPlots;
    listPlots << ui->spectroMeter_waveform_PSD;
    listPlots << ui->spectroMeter_neutron_spectrum_PSD;
    listPlots << ui->spectroMeter_waveform_LBD;
    listPlots << ui->spectroMeter_gamma_spectrum_LBD;
    listPlots << ui->spectroMeter_waveform_LSD;
    listPlots << ui->spectroMeter_neutron_spectrum_LSD;
    listPlots << ui->spectroMeter_gamma_spectrum_LSD;
    for (auto& customPlot : listPlots){
        for (int i = 0; i < customPlot->graphCount(); ++i){
            customPlot->graph(i)->data().clear();
            customPlot->replot(QCustomPlot::rpQueuedReplot);
    ...
    // 从 ComboBox 获取单个文件包对应的时间长度（单位ms）
    int time_per = 40;
    PCIeCommSdk::CaptureTime captureTime = PCIeCommSdk::newCaptureTime;

    //由于每个文件的能谱时长是固定的，所有这里需要根据时刻计算出对应的是第几个文件
    qint32 time1 =ui->spinBox_time1->value();
    quint32 fileIndex = (float)(ui->spinBox_time1->value() + time_per)/ time_per;
...
//                                    tmMeasure);
    ...
    {        
        ...
        if (QFileInfo::exists(filePath) && !mPCIeCommSdk.analyzeHistoryWaveformData(cameraIndex, timeLength, time1 / captureTime, filePath))
        {
            QMessageBox::information(this, tr("提示"), tr("文件格式错误，加载失败！"));
        }

        filePath = QString("%1/%2spec%3.bin").arg(ui->textBrowser_filepath->toPlainText()).arg(joinFilename(cameraIndex)).arg(fileIndex);
        if (QFileInfo::exists(filePath) && !mPCIeCommSdk.analyzeHistorySpectrumData(cameraIndex, 1, time1 / captureTime, filePath))
        {
            QMessageBox::information(this, tr("提示"), tr("文件格式错误，加载失败！"));
        }
    }
...
#include <QtMath>
void OfflineWindow::onWaveform(quint8 timestampIndex, quint8 cameraIndex, QVector<QPair<double,double>>& waveformBytes)
{
 ...
 */
void OfflineWindow::onCalculateDensityPSD(quint8 cameraOrientation, QVector<QPair<double ,double>>& pairs)
{
    QVector<double> x, y;
    QVector<double> z;
    for (auto pair : pairs){
    ...
    emit doPSDPlot(cameraOrientation, x, y, z);
}
 ...
 */
void OfflineWindow::onPSDPlot(quint8 cameraOrientation, const QVector<double>& psd_x, const QVector<double>& psd_y, const QVector<double>& density)
{
...
{
    if (checked){
        ui->spectroMeterPageInfoWidget->setCurrentWidget(ui->spectroMeterPageInfoWidget_page_1);

        //显示相机选项
        ui->label_8->show();
        ui->label_13->show();
        ui->comboBox_horCamera->show();
        ui->comboBox_verCamera->show();
...
{
    if (checked){
        ui->spectroMeterPageInfoWidget->setCurrentWidget(ui->spectroMeterPageInfoWidget_page_2);
...
void OfflineWindow::on_action_cps_triggered(bool checked)
{
    if (checked){
        ui->spectroMeterPageInfoWidget->setCurrentWidget(ui->spectroMeterPageInfoWidget_3);

        //隐藏相机选项
        ui->label_8->hide();
        ui->label_13->hide();
        ui->comboBox_horCamera->hide();
        ui->comboBox_verCamera->hide();
...
void OfflineWindow::onCpsPlot(QMap<quint8/*通道号*/, QVector<QPair<quint16/*时刻*/,quint32/*计数率*/>>> mapPairs)
{
    ...
    for (auto iter = mapPairs.begin(); iter != mapPairs.end(); ++iter){
        for (int group = 0; group < numGroups; ++group) {
            ...
            QCPGraph *graph = nullptr;
            if (group < 11)
                graph = mCpsPlot->graph(QStringLiteral("HC %1").arg(group+1));
            else
                graph = mCpsPlot->graph(QStringLiteral("VC %1").arg(group+1));
            graph->setData(xData, yData);
        }
    }
...
QString OfflineWindow::joinFilename(const int& cameraIndex)
{
    QString filename;
...
void OfflineWindow::onNeutronSpectrum(quint8 timestampIndex, quint8 cameraIndex, QVector<QPair<double,double>>& pairs)
{
    quint8 cameraOrientation = cameraIndex <= 11 ? PCIeCommSdk::CameraOrientation::Horizontal : PCIeCommSdk::CameraOrientation::Vertical;
...
void OfflineWindow::onGammaSpectrum(quint8 timestampIndex, quint8 cameraIndex , QVector<QPair<double,double>>& pairs)
{
    quint8 cameraOrientation = cameraIndex <= 11 ? PCIeCommSdk::CameraOrientation::Horizontal : PCIeCommSdk::CameraOrientation::Vertical;
...
void OfflineWindow::on_comboBox_horCamera_currentIndexChanged(int index)
{
    emit ui->action_analyze->trigger();
}


void OfflineWindow::on_comboBox_verCamera_currentIndexChanged(int index)
{
    emit ui->action_analyze->trigger();
}
