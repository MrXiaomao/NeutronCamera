#ifndef QGAUGEPANEL_H
#define QGAUGEPANEL_H
/**
 * 面板仪表盘控件
 * 1:可设置范围值,支持负数值
...
class QDESIGNER_WIDGET_EXPORT QGaugePanel : public QWidget
#else
class QGaugePanel : public QWidget
#endif

{
    ...
    Q_PROPERTY(double warnThold READ getWarnThold WRITE setWarnThold)
    Q_PROPERTY(double alarmThold READ getAlarmThold WRITE setAlarmThold)
    ...
    Q_PROPERTY(bool pointerStyle READ getPointerStyle WRITE setPointerStyle)
...
public:
    explicit QGaugePanel(QWidget *parent = 0);
    ~QGaugePanel();
...
private:
    ...
    bool pointerStyle;               //指针样式 true-扁平 false-立体
...
public:
    ...
    double getWarnThold()           const;
    double getAlarmThold()          const;
...
public Q_SLOTS:
    ...
    void setWarnThold(double warnThold);
    void setAlarmThold(double alarmThold);
    ...
    //设置指针样式
    void setPointerStyle(bool pointerStyle);
    //设置背景颜色
